...
## **Работа с удаленными репоситориями**
...
Для того чтобы связаться с удалённым репозиторием и забирать из него все изменения, которых у вас пока нет и сохранить их локально, необходимо использовать команду:

	git fetch

Также есть отдельная команда совмещающая git fetch и git merge, т. е. Git вначале забирает изменения из указанного удалённого репозитория, а затем пытается слить их с текущей веткой.

	git pull

## Отправка изменений в удаленный репозиторий

Для установления связи с удалённым репозиторием, вычисления локальных изменений отсутствующих в нём, и собственно их передачи в вышеупомянутый репозиторий, необходимо использовать команду:
